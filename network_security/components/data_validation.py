...
class DataValidation:
    ...
    def check_numerical_columns_exist(self, dataframe:pd.DataFrame) -> bool:
        try:
            if dataframe.select_dtypes(include=np.number).columns.to_list():
                return True
            return False
        except Exception as e:
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            num_cols = self.check_numerical_columns_exist(dataframe=train_dataframe)

            if not num_cols:
                 error_message = f" Numerical columns are missing in the training dataframe. \n"

            num_cols = self.check_numerical_columns_exist(dataframe=test_dataframe)

            if not num_cols:
                error_message = f" Numerical columns are missing in the test dataframe \n"
            ...
            data_validation_artficact = DataValidationArtifact(
            ...
            return data_validation_artficact
